...
// EFF large word list (subset - 1296 words for efficiency, covering dice rolls 11111-66666)
const WORD_LIST = [
...
export default function PassphraseGeneratorClient() {
  ...
  const generate = useCallback(() => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (appendNumber) phrase += separator + getRandomNumber(0, 9999)
      results.push(phrase)
    }
  ...
  const estimateEntropy = () => {
    const wordEntropy = Math.log2(WORD_LIST.length) * wordCount
    const numberEntropy = appendNumber ? Math.log2(10000) : 0
  ...
  return (
    <div>
      <Breadcrumb items={[{ label: 'Home', href: '/' }, { label: 'Passphrase Generator' }]} />

      <h1 className="text-2xl font-bold mb-2 text-gray-900">密码短语生成器 | Passphrase Generator</h1>
      <p className="text-sm text-gray-500 mb-6">Generate secure, memorable passphrases using the EFF word list. Better security than traditional passwords, easier to remember.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Number of Words: {wordCount}</label>
                <input type="range" min={3} max={8} value={wordCount} onChange={e => setWordCount(Number(e.target.value))} className="w-full accent-primary-600" />
                ...
                <div className="flex gap-2">
                  {['-', '.', '_', ' ', '/'].map(s => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-lg border text-sm font-mono ${separator === s ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
                    >
                      {s === ' ' ? '⎵' : s}
                    </button>
                  ))}
              ...
              <label className="flex items-center gap-2 text-sm text-gray-700">
                <input type="checkbox" checked={capitalize} onChange={e => setCapitalize(e.target.checked)} className="rounded accent-primary-600" />
                Capitalize words
              </label>
              <label className="flex items-center gap-2 text-sm text-gray-700">
                <input type="checkbox" checked={appendNumber} onChange={e => setAppendNumber(e.target.checked)} className="rounded accent-primary-600" />
                Append random number
              </label>
            </div>
          </div>

          <button onClick={generate} className="w-full py-2.5 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700 transition-colors">
            Generate Passphrases
          </button>

          {/* Entropy Info */}
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <h3 className="text-sm font-semibold text-gray-800 mb-3">Security Estimate</h3>
            <div className="space-y-2">
              <div className="flex justify-between">
                <span className="text-xs text-gray-500">Entropy</span>
                <span className="text-sm font-mono font-semibold text-primary-600">{entropy} bits</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-gray-500">Crack time (10T/s)</span>
                <span className="text-xs font-semibold text-gray-700">{estimateCrackTime(entropy)}</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                <div
                  className={`h-2 rounded-full ${entropy < 50 ? 'bg-red-500' : entropy < 70 ? 'bg-yellow-500' : 'bg-green-500'}`}
                  style={{ width: `${Math.min(100, entropy)}%` }}
                />
              </div>
              <div className="text-xs text-gray-400">
                {entropy < 50 ? 'Weak — increase word count' : entropy < 70 ? 'Moderate — good for most uses' : 'Strong — excellent security'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-3">
          {passphrases.map((phrase, i) => (
            <div key={i} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-3">
              ...
              <code className="flex-1 text-sm font-mono text-gray-900 tracking-wide select-all break-all">{phrase}</code>
          ...
          {passphrases.length === 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-8 text-center text-gray-400">
              Click "Generate Passphrases" to create secure passphrases
            </div>
          )}
        </div>
      </div>

      <RelatedTools current="passphrase-generator" />
    </div>
  )
}
